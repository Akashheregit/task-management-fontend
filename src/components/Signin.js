...
function Signin() {
 ...
 const usercreate = async (e) => {
  e.preventDefault();
  const response=await fetch (`https://task-management-backend-h1rn.onrender.com/api/auth/createuser`,{
   method: 'POST',
   headers:{
...
if(json.success)
{
 ...
 const handlesubmit = async(e) => {
  ...
  {
    setshow(true);
  const response=await fetch (`https://task-management-backend-h1rn.onrender.com/api/auth/otp`,{
   method: 'POST',
   headers:{
     'Content-Type': 'application/json',
   },
    ...
    <div className="flex items-center h-screen w-full">
      ...
      <span className="block w-full text-xl uppercase font-bold mb-4">Login</span>      
        <form className="mb-4" action="/" method="post">
          ...
          <button className="bg-purple-500 hover:bg-purple-400 text-white uppercase text-sm font-semibold px-4 py-2 rounded" onClick={usercreate}>Sign in</button>
        </form>
    </div>
  </div>
